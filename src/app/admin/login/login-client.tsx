...
export default function LoginClient() {
    ...
    const [state, loginAction, loginPending] = useActionState(
        async (_prev: LoginState, formData: FormData): Promise<LoginState> => {
            ...
            if (!email || !password) return { phase: "login", error: "Введите email и пароль" };
            ...
            if (!res.ok) return { phase: "login", error: "Неверный email/пароль или капча" };
    ...
    const [otpState, otpAction, otpPending] = useActionState(
        async (_prev: LoginState, formData: FormData): Promise<LoginState> => {
            ...
            if (!/^\d{6}$/.test(code)) return { phase: "otp", error: "Введите 6-значный код" };
    ...
    return (
        <div className="login-container">
            <div className="login-box">
                ...
                <div id="errorMessage" className={`error-message ${current.error ? "show" : ""}`}>
                    {current.error ?? ""}
                </div>

                {!isOtp ? (
                    <form action={loginAction}>
                        <button type="button" onClick={handleCreateAdmin}>Create Admin</button>
                        ...
                        <button
                            ...
                            disabled={loginPending || (isProd && siteKey ? !recaptchaToken : false)}
                        >
                    ...
                    <form action={otpAction}>
                        ...
                        <button type="submit" className="login-button" disabled={otpPending}>
                            {otpPending ? "Verifying..." : "Verify"}
                        </button>

                        <p className="login-subtitle" style={{ marginTop: 16, marginBottom: 0 }}>
