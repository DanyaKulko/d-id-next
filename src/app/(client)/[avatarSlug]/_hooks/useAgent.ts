...
import { chatAction, createSessionAction, submitAnswerAction, submitIceAction } from "@/app/actions/agent.actions";

type ConnectionStatus = 'idle' | 'connecting' | 'connected' | 'error';

export const useAgent = (agentId: string, videoRef: React.RefObject<HTMLVideoElement | null>) => {
    const [status, setStatus] = useState<ConnectionStatus>('idle');
    const [isAgentSpeaking, setIsAgentSpeaking] = useState(false);

    const pcRef = useRef<RTCPeerConnection | null>(null);
    const dcRef = useRef<RTCDataChannel | null>(null);
    const idsRef = useRef<{ streamId: string; sessionId: string; chatId: string } | null>(null);
            ...
            dc.onmessage = (event) => {
                ...
                if (msg.includes('stream/started')) {
                    ...
                    if (match) {
                        ...
                        } catch (e) { console.error('Parse videoId error', e); }
                    }
                }
    ...
    const interrupt = useCallback(() => {
        if (dcRef.current && dcRef.current.readyState === 'open' && currentVideoIdRef.current) {
    ...
    return { connect, disconnect: cleanup, speak, interrupt, status, isAgentSpeaking };
};
