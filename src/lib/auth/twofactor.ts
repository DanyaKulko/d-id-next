...
export async function verifyEmail2fa(params: { userId: string; tokenId: string; code: string }) {
    const t = await prisma.twoFactorToken.findFirst({
        where: { id: params.tokenId, userId: params.userId, consumedAt: null, expiresAt: { gt: new Date() } },
    ...
    if (t.attempts >= t.maxAttempts) {
        await prisma.twoFactorToken.update({ where: { id: t.id }, data: { consumedAt: new Date() } });
        return { ok: false as const, reason: "too_many_attempts" as const };
    }

    const ok = crypto.timingSafeEqual(Buffer.from(t.codeHash), Buffer.from(sha256(params.code)));
    if (!ok) {
        await prisma.twoFactorToken.update({ where: { id: t.id }, data: { attempts: { increment: 1 } } });
